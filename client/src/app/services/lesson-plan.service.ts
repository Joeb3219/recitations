...
export class LessonPlanService {
	...
	public async getCourseLessonPlans(course: Course | String) : Promise<LessonPlan[]>{
		...
		return new Promise((resolve, reject) => {
			this.http.get(url).subscribe((result: { data: any[] }) => {
				if(result) resolve((result.data || []).map((data) => new LessonPlan(data)))
				else reject(new Error("No result returned"))
			}, (err) => {
				reject(err)
			})
  ...
  public async deleteLessonPlan(lessonPlanID: string): Promise<LessonPlan>{
    ...
    return new Promise((resolve, reject) => {
      this.http.delete(url).subscribe((result: { data: any }) => {
        if(result) resolve(new LessonPlan(result.data))
        else reject(new Error("No result returned"))
      }, (err) => {
        reject(err)
